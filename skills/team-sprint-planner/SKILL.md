...
model: opus
description: 'Use when the user wants a codebase-grounded, multi-story plan for /team-sprint — "write a plan for team-sprint", "turn this goal/spec into a sprint plan", "decompose this into sprint stories".'
---
...
```bash
TS=~/.claude/skills/team-sprint/scripts
...
1. **Dot-point read-back (mechanical).**
   ```bash
   bash ~/.claude/skills/team-sprint-planner/scripts/plan_readback.sh <plan-path>
   ```
...
- `references/plans/planner-contract-sync-1.md` — the plan that closed the last round of drift
  between this contract doc and team-sprint's actual parser/validator/graph-builder scripts.

## Internal machinery

Not part of the phases above — supporting the scripts and eval harness, not the user-facing flow.

- `scripts/lib.sh` — shared helpers sourced by `scripts/plan_readback.sh` (SCRIPTS resolution,
  logging, tool-presence checks).
- `scripts/tests/run-all.sh` — local CI harness: shellchecks every top-level script, then runs
  every `scripts/tests/*.bats` file (`scripts/tests/plan_readback.bats` covers `plan_readback.sh`)
  under real `bats` if installed, else `scripts/tests/lib/bats-fallback.sh`'s plain-bash shim.
- `evals/evals.json` — eval prompts exercising this skill end-to-end against real repos.
