# graphify pipeline — full step commands

The commands and detailed rules for each pipeline step. `SKILL.md` holds the step
skeleton and the hard guards; load the section you need here when running that step.

Available helper scripts (beside this skill, under `scripts/`):
- `scripts/corpus-summary.sh [detect-json]` — Step 2 post-detect logic, mechanised: prints the corpus summary and a size verdict (`stop` / `narrow` / `no-cluster` / `proceed`).
- `scripts/estimate-agents.sh [uncached-file]` — Step 3 Part B dispatch estimate, mechanised: reads `.graphify_uncached.txt` and prints the files/agents/time estimate line.
- `scripts/chunk-check.sh <expected-count>` — Step B3 collection check, mechanised: verifies each chunk JSON exists and parses; exits 1 when more than half failed.
...
Then act on it (or run `scripts/corpus-summary.sh`, which prints this summary and the verdict mechanically):
...
The API-key guard in SKILL.md applies here in full: no key is ever required, and code is extracted structurally (AST) with no LLM at all — a code-only corpus (the common `/graphify .` on a repo) skips semantic extraction entirely, so it needs nothing here: go straight to Part A and skip Part B. Semantic extraction (only for docs, papers, and images) uses Gemini **only if** `GEMINI_API_KEY`/`GOOGLE_API_KEY` is already set; otherwise the host agent itself is the LLM.
...
Before dispatching subagents, print a timing estimate (or run `scripts/estimate-agents.sh`, which prints the same line from `.graphify_uncached.txt`):
...
Wait for all subagents. For each result (or run `scripts/chunk-check.sh <expected-count>`, which performs these checks and prints a proceed/abort verdict):
