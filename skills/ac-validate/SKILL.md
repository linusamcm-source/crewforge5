...
If invoked as part of a multi-agent sprint (e.g., from `/team-sprint`), deliver the
report to the team-lead via `SendMessage` after writing it. Inline-only descriptions
of findings are not sufficient — the file path AND the SendMessage delivery are both
required so the lead can route fixes downstream.
