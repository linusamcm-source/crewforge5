...
`bash $SCRIPTS/state.sh init|read|update|advance-phase` is the only writer. `$SCRIPTS/schemas/state.schema.json` is the machine schema. If a sprint dies mid-flight, the next invocation discovers it by scanning `$ART/../*/state.json` and resumes the one whose `plan_path` matches.
...
## Bundle manifest — internal machinery
...
- **Dev docs** (`references/docs/`, history not runtime): ADRs `adr/README.md` + `001-subskill-extension-surface.md`; the sprint plans that built this skill under `references/docs/plans/` — `sprint-team-sprint-mech-refactor.md`, `sprint-team-sprint-mech-refactor-v2.md`, `sprint-team-sprint-mech-refactor-v3.md`, `v2-ferrari-rewrite.md`, `graph-mode-hardening.md`, `findings-loop-1.md`, `recon-harness-1.md`, `workflow-adoption-1.md`.
