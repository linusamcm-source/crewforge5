...
> a fact to check, not a preference to weigh — do not choose between the two paths on judgment. `$SKILL/workflows/story-executor.workflow.js`
> runs Phases 3–5 for one story as a single `Workflow` call: `Workflow({ scriptPath: "<skill>/workflows/story-executor.workflow.js",
