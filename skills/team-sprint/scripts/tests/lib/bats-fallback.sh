...
# `skip`: bats halts the test body at the skip point — nothing after it may
# run. Under fallback each test body executes in a subshell inside
# __bats_fb_run_one, so `exit 0` cleanly aborts the body and reports the
# test as passed. A `return 0` would NOT abort it: under the common
# `[ -f x ] || skip "reason"` guard form the body would keep executing.
# Only defined under fallback — under real bats the native skip must win.
if [ -n "${_BATS_FALLBACK_ACTIVE:-}" ] && ! declare -F skip >/dev/null 2>&1; then
  skip() {
    if [ "$#" -gt 0 ]; then
      printf '  skip: %s\n' "$*" >&2
    fi
