...
- The `recon.sh` block added to `recon-instruments.md` is guarded by the same executable test the graphify block at `recon-instruments.md:42-43` uses — `RS=${CREWFORGE_ROOT}/skills/team-sprint/scripts/recon.sh; if [ -x "$RS" ]; then ...; fi` — and the surrounding text states the fallback when it is absent, asserted by grepping the post-edit file for `-x "$RS"`.
