...
**Source:** `$CLAUDE_CONFIG_DIR/union.md` (Graphify vs GitNexus vs CodeGraph comparison), reviewed
...
**On `$CLAUDE_CONFIG_DIR` itself the CodeGraph half of this harness is inert, and the plan assumes it.**
Verified this session: `codegraph status` in `$CLAUDE_CONFIG_DIR` prints `⚠ Not initialized`, and
...
- **The probe records index state, not just binary presence.** A present binary over an
  ...
  in `$CLAUDE_CONFIG_DIR`: prints `⚠ Not initialized`, exits 0) — and stores the result as `indexed`
...
- `skills/team-sprint/team-sprint.config.yaml.example:173-178` `graphify_max_age_minutes` (produces the *documented* default of 240 for graphify's `fresh` vs `stale:<n>m` split — verified 2026-07-27 that `skills/team-sprint/team-sprint.config.yaml` does not exist and the live `$CLAUDE_CONFIG_DIR/team-sprint.config.yaml` sets no `graphify_max_age_minutes` at all, so no config file is a producer of defaults and `recon.sh` supplies 240 itself)
- `skills/team-sprint/team-sprint.config.yaml.example:141-146` and `$CLAUDE_CONFIG_DIR/team-sprint.config.yaml:30` `repomix_max_age_minutes` (produces the threshold for the repomix-served `text` intent; documented default and live value are both 240, and `recon.sh` hardcodes 240 for the empty return)
- `$CLAUDE_CONFIG_DIR/CLAUDE.md` "Never run `find` from `/` or `~`" (produces the constraint on the file-count fallback)
...
code, which is 0 even when the project is not initialised (verified 2026-07-27 in `$CLAUDE_CONFIG_DIR`:
...
(`$CLAUDE_CONFIG_DIR/team-sprint.config.yaml:32`, verified 2026-07-28) to `graphify: auto`.
...
### Boundaries:
- the target repo's `.gitignore` (RH4 appends `.recon/` to it — a file this story does not own and which may be a whitelist, as `$CLAUDE_CONFIG_DIR/.gitignore` is: there `/*` ignores everything, `git check-ignore -v .recon/` already answers `.gitignore:8:/*` (verified 2026-07-27), and an appended `.recon/` line would be redundant, not effective)
...
- `$CLAUDE_CONFIG_DIR/team-sprint.config.yaml`
...
- [ ] The `recon` block is present in the live `$CLAUDE_CONFIG_DIR/team-sprint.config.yaml`, asserted by reading the file (it is untracked and `.gitignore`-matched, so it appears in no `git diff`) — otherwise every key resolves to its hardcoded default for the whole sprint.
...
- The `recon.sh` block added to `recon-instruments.md` is guarded by the same executable test the graphify block at `recon-instruments.md:42-43` uses — `RS=${CREWFORGE5_ROOT}/skills/team-sprint/scripts/recon.sh; if [ -x "$RS" ]; then ...; fi` — and the surrounding text states the fallback when it is absent, asserted by grepping the post-edit file for `-x "$RS"`.
